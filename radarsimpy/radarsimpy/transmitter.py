...
class Transmitter:
    ...
    **Waveform**
    ...
    def get_frequency_at_time(self):
        """
        Calculate the frequency at every instant of time.

        :return:
            An array of tuples where each tuple contains a time and its corresponding frequency.
        :rtype: list of tuples (time, frequency)
        """
        frequencies = []

        for i in range(len(self.waveform_prop["t"])):
            time = self.waveform_prop["t"][i]
            frequency = self.waveform_prop["f"][i] + self.waveform_prop["f_offset"][i % len(self.waveform_prop["f_offset"])]
            frequencies.append((time, frequency))
